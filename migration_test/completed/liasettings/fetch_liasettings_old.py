...
"""Step 1 of the LIASETTINGS migration: fetch `liasettings` from the OLD Content
API v2.1.

This mirrors the production roll-down logic in `acit/acit.py`:
  - topology via `accounts.authinfo` (aggregators vs standalone),
  - for each AGGREGATOR (MCA): `liasettings.get(merchantId=agg, accountId=agg)`
    -> `settings`, then `liasettings.list(merchantId=agg)` -> `children[]`
    (result_path='resources'), written as one `{settings, children[]}` envelope.
  - for each STANDALONE leaf: `liasettings.get(merchantId=parent, accountId=leaf)`
...
`migration_test/env.local.sh`. Output goes to `migration_test/old_liasettings/`.
...
_OUT_DIR = pathlib.Path(__file__).parent / "old_liasettings"
...
def _write_envelope(account_id, envelope):
    path = _OUT_DIR / f"{account_id}_liasettings.json"
...
def main():
    ...
    for agg in sorted(aggregator_ids):
        settings = next(_get_results(
            api.liasettings,
            {"merchantId": agg, "accountId": agg},
            "get", "", {"accountId": agg}, is_scalar=True,
        ))
        ...
        envelope = {"settings": settings, "children": children}
        _write_envelope(agg, envelope)
        n_cs = len(settings.get("countrySettings", []))
        print(f"  MCA {agg}: settings(countrySettings={n_cs}), {len(children)} children")
        summary["aggregators"].append({
            "account_id": agg,
            "self_country_settings": n_cs,
            "n_children": len(children),
            "children_with_country_settings": sum(
                1 for c in children if c.get("countrySettings")
            ),
        })
    ...
    for leaf in sorted(standalone_ids):
        try:
            settings = next(_get_results(
                api.liasettings,
                {"merchantId": leaf, "accountId": leaf},
                "get", "", {"accountId": leaf}, is_scalar=True,
            ))
        except gapi_errors.HttpError as e:
            print(f"  standalone {leaf}: get failed ({e.status_code}); skipping")
            continue
        envelope = {"settings": settings, "children": []}
        _write_envelope(leaf, envelope)
        n_cs = len(settings.get("countrySettings", []))
        print(f"  standalone {leaf}: countrySettings={n_cs}")
        summary["standalone"].append({"account_id": leaf, "country_settings": n_cs})

    (_OUT_DIR / "summary.json").write_text(json.dumps(summary, indent=2))
    total = len(summary["aggregators"]) + len(summary["standalone"])
    print(f"\nDone. {total} account(s) with liasettings -> {_OUT_DIR}")
    if not total:
        print("!! No liasettings found. Check MERCHANT_IDS / ADMIN rights / credential access.")
